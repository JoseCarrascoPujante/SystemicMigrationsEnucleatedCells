function imageJ_emu(coordinates)
    ...
    slider = uislider(fig, 'Orientation','vertical','Value', 1,...
        'Position',[1060 30 1 690], 'ValueChangingFcn', @updateImage);
    ...
    while f <= length(f_n)
        ...
        while i <= size(i_n,1)
            ...
            j = 1;
            msgbox('Switched to previous experiment, loading frames:')
            loadExp()            
        end
        i = 1;
        f = f + 1;
    ...
    function [pc,pf,pm,pt] = plotData()
        switch exist("pc","var") && exist("pf","var") && exist("pm","var") && exist("pt","var")
            case 0
