function Figure7(tracks,T,destination_folder)
    %% Layouts
    ...
    yl1 = ylim();
    xl1 = xlim();
    text(xl1(1)+1,yl1(1)+.65,strcat('\gamma=',num2str(round(gamma,2))),"FontSize",10)
    ...
    yl2 = ylim();
    xl2 = xlim();
    text(xl2(1)+1,yl2(1)+.5,strcat('\gamma=',num2str(round(gamma,2))),"FontSize",10)
    ...
    ylim([yl1(1) yl1(2)]);
    xlim([xl1(1) xl1(2)]);
    text(xl1(1)+1,yl1(1)+1.85,strcat('\gamma=',num2str(round(gamma,2))),"FontSize",10)
    ...
    ylim([yl2(1) yl2(2)]);
    xlim([xl2(1) xl2(2)]);
    text(xl2(1)+1,yl2(1)+1.85,strcat('\gamma=',num2str(round(gamma,2))),"FontSize",10)
    ...
    for v = 1:length(versions)
        if  contains(versions(v).name, 'Figure7'+wildcardPattern+'.svg')
            gabs = gabs + 1 ;
        end
    end
    
    disp(strcat(num2str(gabs-1),' Figure7 files found'))
    print(fig,'-vector','-dsvg',[destination_folder '\Figures\Figure7(',num2str(gabs),')' '.svg'])
    print(fig,'-image','-djpeg','-r400',[destination_folder '\Figures\Figure7(',num2str(gabs),')' '.jpg'])    
