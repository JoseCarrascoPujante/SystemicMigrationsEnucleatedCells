function Figure3(tracks,destination_folder)
    ...
    for v = 1:length(versions)
        if  contains(versions(v).name, 'Figure3'+wildcardPattern+'.svg')
            gabs = gabs + 1 ;
        end
    end
    
    disp(strcat(num2str(gabs-1),' Figure3 files found'))

    print(fig,'-vector','-dsvg',[destination_folder '\Figures\Figure3(',num2str(gabs),')' '.svg'])
    print(fig,'-image','-djpeg','-r400',[destination_folder '\Figures\Figure3(',num2str(gabs),')' '.jpg'])
