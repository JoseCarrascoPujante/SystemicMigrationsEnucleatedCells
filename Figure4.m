function Figure4(tracks,T,destination_folder)
    ...
    for v = 1:length(versions)
        if  contains(versions(v).name, 'Figure4'+wildcardPattern+'.svg')
            gabs = gabs + 1 ;
        end
    end
    
    disp(strcat(num2str(gabs-1),' Figure4 files found'))
    ...
    for iFig = length(FigList):-1:1
        ...
        print(FigHandle,'-vector','-dsvg',[destination_folder '\Figures\Figure4(',num2str(gabs),')' '.svg'])
        print(FigHandle,'-image','-djpeg','-r400',[destination_folder '\Figures\Figure4(',num2str(gabs),')' '.jpg'])
    end
end