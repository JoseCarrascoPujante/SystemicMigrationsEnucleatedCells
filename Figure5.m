function Figure5(tracks,T,destination_folder)
    ...
    for f = 1:length(field_names)
        ...
        line([sdatamean-sdatastd sdatamean+sdatastd],[0 0],'Color','b',...
            'LineWidth',.5)
        line([sdatamean-sdatastd+.002 sdatamean-sdatastd],[0 0],'Color','b',...
            'LineWidth',5)
        line([sdatamean+sdatastd sdatamean+sdatastd+.002],[0 0],'Color','b',...
            'LineWidth',5)
    ...
    for v = 1:length(versions)
        if  contains(versions(v).name, 'Figure5'+wildcardPattern+'.svg')
            gabs = gabs + 1 ;
        end
    end
    
    disp(strcat(num2str(gabs-1),' Figure5 files found'))
    print(fig,'-vector','-dsvg',[destination_folder '\Figures\Figure5(',num2str(gabs),')' '.svg'])
    print(fig,'-image','-djpeg','-r400',[destination_folder '\Figures\Figure5(',num2str(gabs),')' '.jpg'])
